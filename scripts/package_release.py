...
create_or_identical(dist/'package.json',json.dumps({'name':'cello-notebook-portable','private':True,'version':'0.4.0','scripts':{'start':'node start-server.cjs'}},indent=2))
files=sorted(p for p in dist.rglob('*') if p.is_file())
assert not any('test-backup' in p.name or '教材' in p.name for p in files)
output=root/'cello-notebook-v0.4.0.zip'
...
with zipfile.ZipFile(output) as z:
 ...
 assert len([n for n in z.namelist() if n.endswith('.musicxml')])==28
