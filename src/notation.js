...
function renderSystem(l,start,count,layers={},selection=null,interactive=true,mobile=false){
 ...
 for(let i=0;i<count;i++){
  ...
  points.forEach(({x,y,event:e},ni)=>{
   ...
   const r=rhythm(e.beats);
   if(p){const accidentalKey=p.step+p.octave,previous=accidentals.has(accidentalKey)?accidentals.get(accidentalKey):keyAlter(l,p.step);if(p.alter!==previous&&!['stop','continue'].includes(e.tie))s+=glyph(p.alter===1?'E262':p.alter===-1?'E260':'E261',x-23,y);accidentals.set(accidentalKey,p.alter);
   ...
   }else s+=glyph(({.125:'E4E8',.25:'E4E7',.5:'E4E6',1:'E4E5',2:'E4E4',4:'E4E3'})[r.base],x,96);
...
function musicxml(l,answers=false,tempo=l.tempo){
 C.validateLesson(l);const bars=l.bars.map((bar,i)=>{
  ...
  if(i===0){inner+=`<attributes><divisions>8</divisions><key><fifths>${l.fifths}</fifths><mode>${l.mode||'major'}</mode></key><time><beats>${l.meter[0]}</beats><beat-type>${l.meter[1]}</beat-type></time><clef><sign>F</sign><line>4</line></clef></attributes><direction placement="above"><direction-type><words>${esc(l.tempoLabel||'Practice tempo')}</words></direction-type></direction><direction placement="above"><direction-type><metronome><beat-unit>quarter</beat-unit><per-minute>${tempo}</per-minute></metronome></direction-type><sound tempo="${tempo}"/></direction>`;if(l.dynamic)inner+=`<direction placement="below"><direction-type><dynamics><${l.dynamic}/></dynamics></direction-type></direction>`;}
  let beat=0;bar.forEach((e,j)=>{if(e.hold)inner+=`<direction placement="below"><direction-type><words>${esc(e.hold)}</words></direction-type></direction>`;
   if(e.extension)inner+=`<direction placement="above"><direction-type><words>${e.extension==='back'?'A 弦低 1 指（后伸）':'G 弦 4 指前伸半音'}</words></direction-type></direction>`;
   if(e.expression)inner+=`<direction placement="below"><direction-type><words>${esc(e.expression)}</words></direction-type></direction>`;
   const p=C.pitchInfo(e.pitch),r=rhythm(e.beats),tieStart=['start','continue'].includes(e.tie),tieStop=['stop','continue'].includes(e.tie);let n=p?`<pitch><step>${p.step}</step>${p.alter?`<alter>${p.alter}</alter>`:''}<octave>${p.octave}</octave></pitch>`:'<rest/>';
   n+=`<duration>${e.beats*8}</duration>${tieStop?'<tie type="stop"/>':''}${tieStart?'<tie type="start"/>':''}<type>${r.type}</type>${r.dotted?'<dot/>':''}`;if(p&&e.beats<4)n+='<stem>down</stem>';
   ...
   if(technical||articulations||tieStart||tieStop)n+=`<notations>${tieStop?'<tied type="stop"/>':''}${tieStart?'<tied type="start"/>':''}${technical?`<technical>${technical}</technical>`:''}${articulations?`<articulations>${articulations}</articulations>`:''}</notations>`;
