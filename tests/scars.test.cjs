...
test('辅助指法和 D 大调信息写入输出',()=>{assert.equal(l.fifths,2);assert.match(N.renderSystem(l,0,4,{names:true,strings:true,fingers:true,jianpu:true}),/C 弦/);assert.match(N.musicxml(l,true),/<fingering>0<\/fingering>/);});
