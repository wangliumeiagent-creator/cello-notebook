...
(async()=>{const b=await chromium.launch({headless:true,executablePath:process.env.BROWSER_PATH||'C:/Program Files (x86)/Microsoft/Edge/Application/msedge.exe'});try{
 ...
 assert.equal(await p.locator('#title').textContent(),'天空之城主旋律');assert.match(await p.locator('#chips').textContent(),/D 小调/);assert.equal(await p.locator('#score .score-line').count(),26);
 ...
 await p.locator('#score .score-line').nth(49).screenshot({path:'qa/laputa/mobile-ending.png'});
