...
(async()=>{const b=await chromium.launch({headless:true,executablePath:process.env.BROWSER_PATH||'C:/Program Files (x86)/Microsoft/Edge/Application/msedge.exe'});try{
 ...
 assert.equal(await p.locator('#title').textContent(),'时之伤痕');assert.equal(await p.locator('#score .score-line').count(),8);assert.equal(await p.locator('.nav-link').count(),16);assert.equal(await p.locator('#score .note-event').count(),127);
